...
Electron 기반 데스크탑 위젯. Claude Code 스킬 런처 + 워크스페이스 대시보드.
...
```
┌──────────────────────┐
│  🫧 Sophia Hub       │  ← 드래그 가능, Alt+Space 토글
├──────────────────────┤
│ 🏠 📋 📜 📝 ···     │  ← 탭 바
├──────────────────────┤
│                      │
...
Sophia Hub의 핵심은 **Claude Code Custom Skills**를 GUI에서 관리하고 원클릭 실행하는 것이다.
...
name: 스킬이름
description: 한 줄 설명
category: dev           # dev | idea | work | more (생략 시 자동 감지)
---
...
스킬은 4개 카테고리로 자동 분류된다:

| 카테고리 | 조건 | 키워드 예시 |
|---------|------|-----------|
| **dev** | 기술/개발 키워드 2개+ | `프로젝트`, `경로:`, `기술 스택`, `git`, `npm`, `react`, `python` |
| **work** | 업무 키워드 2개+ | `업무`, `테스트`, `버그`, `리포트`, `회고`, `KPT`, `PRD` |
| **idea** | 아이디어 키워드 2개+ | `아이디어`, `브레인스토밍`, `주제 선택` |
| **more** | 위 조건 미충족 | (기본값) |
...
| 앱 설정/상태 | `%APPDATA%/sophia-hub/store.json` | JSON key-value |
...
```bash
git clone https://github.com/ko5439625/sophia-hub.git
cd sophia-hub
npm install
```
...
→ `release/win-unpacked/Sophia Hub.exe`
